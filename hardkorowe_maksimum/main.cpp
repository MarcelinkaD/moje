//https://szkopul.edu.pl/c/oki-poziom-2-202425/p/hma/
#include <bits/stdc++.h>
...
ll maximum(int akt_wie, int pocz_odp, int kon_odp, int pocz_pyt, int kon_pyt) {
    if (pocz_pyt <= pocz_odp && kon_odp <= kon_pyt)
    ...
    przepchnij(akt_wie, kon_odp - pocz_odp + 1);
    int srodek = (pocz_odp + kon_odp) / 2;
    return max(maximum(2 * akt_wie, pocz_odp, srodek, pocz_pyt, kon_pyt),
               maximum(2 * akt_wie + 1, srodek + 1, kon_odp, pocz_pyt, kon_pyt));
}
