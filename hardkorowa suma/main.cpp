//https://szkopul.edu.pl/c/oki-poziom-2-202425/p/hsu/
//drzewo przedzialowe przedzial-przedzial
#include <bits/stdc++.h>
...
void przepchnij(int akt_wie, int rozmiar){
    if (przep[akt_wie] != 0){
        drzewo[2 * akt_wie] += przep[akt_wie] * (rozmiar / 2);
        drzewo[2 * akt_wie + 1] += przep[akt_wie] * (rozmiar / 2);
...
void dodaj(int akt_wie, int pocz_odp, int kon_odp, int pocz_pyt, int kon_pyt, ll var){
    if (pocz_pyt <= pocz_odp && kon_odp <= kon_pyt){
        drzewo[akt_wie] += var * (ll)(kon_odp - pocz_odp + 1);
        przep[akt_wie] += var;
        return;
    }
    ...
    drzewo[akt_wie] = drzewo[2 * akt_wie] + drzewo[2 * akt_wie + 1];
}
